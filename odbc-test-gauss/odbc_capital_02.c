...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
  {
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS odbc_capital_02",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_capital_02(omg varchar(20))",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select  c.relname as Name from pg_catalog.pg_class c where relname='odbc_capital_02'",SQL_NTS);  
	...
	SQLLEN len = 0;	
    SQLBindCol(V_OD_hstmt,1,SQL_C_CHAR, &V_OD_buffer,150,&len);	
    ...
    while(V_OD_erg != SQL_NO_DATA)
    {
	 ...
	 SQLGetData(V_OD_hstmt,1,SQL_C_ULONG,&V_ID,0,&len);
