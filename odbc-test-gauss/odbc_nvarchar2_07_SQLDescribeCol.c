...
int main( )
{
  ...
  /*                                                               */
  /*****************************************************************/
   rc = SQLExecDirect(hStmt,"drop table IF EXISTS  odbc_nvarchar2_07",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     {
     	goto exit;
     }
   rc = SQLExecDirect(hStmt,"create table odbc_nvarchar2_07(id int PRIMARY KEY,name nvarchar2(30)) ",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))  
     {
     	goto exit;
     }
   rc = SQLExecDirect(hStmt,"insert into odbc_nvarchar2_07 values(2,'SQLDescribeCol')",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     {
     	goto exit;
     } 	
   rc = SQLExecDirect(hStmt,"select * from odbc_nvarchar2_07",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     {
     	goto exit;
     } 
