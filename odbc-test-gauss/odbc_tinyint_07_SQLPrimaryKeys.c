/*
测试odbc支持tinyint类型
test: SQLPrimaryKeys()函数,字段类型为tinyint的列作为主键的时候，能查询出来
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main( )
{
   ...
   SQLCHAR table[200] = "ODBC_TINYINT_07";   
  ...
  /*                                                               */
  /*****************************************************************/
   rc = SQLExecDirect(hStmt,"drop table IF EXISTS  odbc_tinyint_07",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     {
     	goto exit;
     }
   rc = SQLExecDirect(hStmt,"create table odbc_tinyint_07(id tinyint PRIMARY KEY) distribute by replication ",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))  
     {
     	goto exit;
     }
   rc = SQLExecDirect(hStmt,"insert into odbc_tinyint_07 values(100)",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     {
     	goto exit;
     } 		
