...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
  {
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS odbc_sm3",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_sm3(result varchar(20))",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	SQLExecDirect(V_OD_hstmt,"insert into odbc_sm3 values('connected success')",SQL_NTS);	

	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select *  from odbc_sm3",SQL_NTS);  
