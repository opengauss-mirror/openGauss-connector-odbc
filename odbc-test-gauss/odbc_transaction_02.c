...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
  {
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS odbc_transaction_02",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_transaction_02(name varchar(20))",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	SQLExecDirect(V_OD_hstmt,"insert into odbc_transaction_02 values('hehe')",SQL_NTS);
	
    SQLExecDirect(V_OD_hstmt,"savepoint a;",SQL_NTS);  		

	SQLExecDirect(V_OD_hstmt,"update odbc_transaction_02 set name='haha' where name = 'hehe'",SQL_NTS);	
		
	SQLExecDirect(V_OD_hstmt,"rollback to a",SQL_NTS);
	//ROLLBACK
	SQLEndTran(SQL_HANDLE_ENV,V_OD_Env,SQL_COMMIT);
	
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select name from odbc_transaction_02",SQL_NTS);  
