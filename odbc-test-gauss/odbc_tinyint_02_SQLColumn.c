/*
测试odbc支持tinyint类型
test: SQLColumn()函数,可以获取tinyint的dateType typename columnSize sqlDateType等
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main( )
{
   ...
   SQLCHAR table[200] = "ODBC_TINYINT_02_SQLCOLUMN";
   SQLCHAR col[200] = "ID";
  ...
  /*****************************************************************/
   ...
   rc = SQLExecDirect(hStmt,"drop table IF EXISTS  odbc_tinyint_02_SQLColumn",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     goto exit;
	 
   rc = SQLExecDirect(hStmt,"create table odbc_tinyint_02_SQLColumn(id tinyint,count int)",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     goto exit;

   rc = SQLExecDirect(hStmt,"insert into odbc_tinyint_02_SQLColumn values(1,3696)",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
     goto exit;
