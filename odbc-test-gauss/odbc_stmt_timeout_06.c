/*
测试odbc支持语句执行超时连接设置
test: SQLSetStmtAttr()函数设定5秒超时,三条SQLExecDirect执行，第一个未超时的语句语法错误，报错信息正确，第二个超时被终止，报错信息正确。
*/
#include <stdlib.h>
#include <stdio.h>
#include <sql.h>
...
int main(int argc,char *argv[])
{
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_timeout_06_01(tm int);",SQL_NTS); 
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
	{
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select pg_sleep(10);",SQL_NTS);         //create function  
