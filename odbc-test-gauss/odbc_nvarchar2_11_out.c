...
int main( )
{
  ...
  /*****************************************************************/ 
   rc = SQLExecDirect(hStmt,"drop table  IF EXISTS odbc_nvarchar2_11_manycol",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg);     
	 goto exit;
	} 
   rc = SQLExecDirect(hStmt,"create table odbc_nvarchar2_11_manycol(id tinyint,ft float,identi char(20),omg nvarchar2(5),tm timestamp)",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg);     
     goto exit;
	...
	/* tinyint [0,255] */
   rc = SQLExecDirect(hStmt,"insert into odbc_nvarchar2_11_manycol values(228,2.88,'ni hao','nihaomaas','2013-07-23 13:55:55')",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg); 
    goto exit;
   } 
   rc = SQLExecDirect(hStmt,"select omg from odbc_nvarchar2_11_manycol",SQL_NTS);
   ...
   //test SQLBindCol
   rc = SQLBindCol(hStmt,1,SQL_C_CHAR, (SQLPOINTER)&omg,0,&cbSID);
