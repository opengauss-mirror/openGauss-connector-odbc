...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  // 3. Connect to the datasource "gaussdb" 
	...
	rc = SQLExecDirect(hstmt,"drop table IF EXISTS FVT_INTERFACE.odbc_SQLNumParams",SQL_NTS);
	if ((!SQL_SUCCEEDED(rc)))
	{
   ...
   rc = SQLExecDirect(hstmt,"create table FVT_INTERFACE.odbc_SQLNumParams(tm time)",SQL_NTS);
	if ((!SQL_SUCCEEDED(rc)))
	{
		printf("Error  SQLExecDirect %d\n",rc);
		SQLGetDiagRec(SQL_HANDLE_STMT, hstmt,1, 
  ...
  SQLPrepare(hstmt,"insert into FVT_INTERFACE.odbc_SQLNumParams values(?)",SQL_NTS);
  SQLSMALLINT num;
