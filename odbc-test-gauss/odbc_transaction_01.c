...
int main(int argc,char *argv[])
{
	...
	*/
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS odbc_transaction_01",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_transaction_01(name varchar(20))",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	SQLExecDirect(V_OD_hstmt,"insert into odbc_transaction_01 values('haha')",SQL_NTS);
	
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select name from odbc_transaction_01",SQL_NTS);  
	...
	//ROLLBACK
	SQLEndTran(SQL_HANDLE_ENV,V_OD_Env,SQL_ROLLBACK);
   ...
   if (V_OD_erg != SQL_ERROR)
    {
        printf("Expected: SQL_ERROR with rollback, but not(%d).\n", V_OD_erg);
        return -1;
    }
