/*
测试标识符大小的适配
test: 创建模式，模式名为小写，从系统表中查询显示该模式，where子句模式名以大写来限定
*/
#include <stdio.h>
#include <sql.h>
#include <sqlext.h>
...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
  {
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop schema IF EXISTS ODBC_CAPITAL_04 cascade",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create SCHEMA odbc_capital_04",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"SELECT n.nspname AS Name FROM pg_catalog.pg_namespace n WHERE n.nspname ~'^(odbc_capital_04)'",SQL_NTS);  
