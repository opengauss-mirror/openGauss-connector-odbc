...
int main( )
{
  ...
  /*****************************************************************/ 
   rc = SQLExecDirect(hStmt,"drop table  IF EXISTS odbc_tinyint_10_manycol",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg);     
	 goto exit;
	} 
   rc = SQLExecDirect(hStmt,"create table odbc_tinyint_10_manycol(id tinyint,ft float,identi char(20),omg varchar(20),tm timestamp)",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg);     
     goto exit;
	...
	/* tinyint [0,255] */
   rc = SQLExecDirect(hStmt,"insert into odbc_tinyint_10_manycol values(228,2.88,'ni hao','nihao','2013-07-23 13:55:55')",SQL_NTS);
   if (!SQL_SUCCEEDED(rc))
   { SQLGetDiagRec(SQL_HANDLE_STMT,hStmt,1, errStat,&errInfo,errMsg,100,&errCb);
     printf("%s \n",errMsg);   
    goto exit;
   } 
   rc = SQLExecDirect(hStmt,"select id from odbc_tinyint_10_manycol",SQL_NTS);
