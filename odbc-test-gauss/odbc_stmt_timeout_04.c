/*
测试odbc支持语句执行超时连接设置
test: SQLSetStmtAttr()函数设定5秒超时,三条SQLExecDirect执行，第二个超时被终止，第一个和第三个未超时的语句执行生效。
*/
#include <stdlib.h>
#include <stdio.h>
#include <sql.h>
...
int main(int argc,char *argv[])
{
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS odbc_timeout_04_01;",SQL_NTS);
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
	{
		printf("drop error");
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table odbc_timeout_04_01(tm int);",SQL_NTS);
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
	{
		printf("create error");
		SQLGetDiagRec(SQL_HANDLE_STMT, V_OD_hstmt,1, V_OD_stat,&V_OD_err,V_OD_msg,100,&V_OD_mlen);
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"insert into odbc_timeout_04_01 values(10);select pg_sleep(10);",SQL_NTS);         //create function  
    if (V_OD_erg != SQL_ERROR)
    {
        printf("Expected: SQL_ERROR with timeout, but not(%d).\n", V_OD_erg);
        exit(-1);
    }
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select * from odbc_timeout_04_01;",SQL_NTS); 		
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
	{
		printf("select error");
		SQLGetDiagRec(SQL_HANDLE_STMT, V_OD_hstmt,1, V_OD_stat,&V_OD_err,V_OD_msg,100,&V_OD_mlen);
