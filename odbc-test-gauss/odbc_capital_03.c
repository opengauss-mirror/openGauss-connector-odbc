/*
测试标识符大小的适配
test: 创建列名时为小写，查询系统表显示列名以大写方式存储。
*/
#include <stdio.h>
#include <sql.h>
#include <sqlext.h>
...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
  {
	...
	//SQLExecDirect
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"drop table IF EXISTS ODBC_CAPITAL_03",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"create table ODBC_CAPITAL_03(omg varchar(20))",SQL_NTS);  
	if ((V_OD_erg != SQL_SUCCESS) && (V_OD_erg != SQL_SUCCESS_WITH_INFO))
    {
	...
	V_OD_erg = SQLExecDirect(V_OD_hstmt,"select column_name from all_tab_columns where table_name='odbc_capital_03'",SQL_NTS);  
